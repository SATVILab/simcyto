...
.simCytUsesUpperRatioCorrection <- function(transformationFunc) {
  ...
  calcGamma <- get0("calc_gamma", mode = "function", inherits = FALSE)
  calcSkew <- get0("calc_skew", mode = "function", inherits = FALSE)

  isTRUE(transName %in% c("gamma", "skew")) ||
    (is.function(calcGamma) && isTRUE(identical(transformationFunc, calcGamma))) ||
    (is.function(calcSkew) && isTRUE(identical(transformationFunc, calcSkew)))
}
...
>>>>>>> origin/main
  ...
  if (any(!is.finite(out))) {
    warning(
      "Non-finite values generated by the transformation were coerced to zero.",
      call. = FALSE
    )
    out[!is.finite(out)] <- 0
  }
